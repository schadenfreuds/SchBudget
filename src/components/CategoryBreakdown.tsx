    ...
    <div className="bg-white rounded-xl border border-zinc-200 p-5 shadow-xs">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-zinc-100">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-base font-bold text-zinc-900">Nereye Ne Kadar Harcadık?</h3>
            <p className="text-xs text-zinc-500">Bu ayki sabit faturalar ve tüm harcamaların kategori dağılımı</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <span className="text-xs sm:text-sm font-bold text-zinc-800 bg-zinc-100 px-3 py-1 rounded-lg">
      ...
      {totalSpent === 0 ? (
        <div className="py-12 text-center">
          ...
          <p className="text-sm font-semibold text-zinc-700">Bu ay için henüz harcama kaydı bulunmuyor.</p>
          <p className="text-xs text-zinc-400 mt-1">Fatura veya günlük harcama eklemek için Muhasebe sekmesine geçebilirsiniz.</p>
          {onGoToAccounting && (
            <button
              onClick={onGoToAccounting}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 pt-4">
          {sortedCategories.map(item => (
            <div key={item.id} className="p-2.5 rounded-lg hover:bg-zinc-50 transition space-y-1.5 border border-zinc-100/60">
              <div className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-2 font-semibold text-zinc-800 truncate">
